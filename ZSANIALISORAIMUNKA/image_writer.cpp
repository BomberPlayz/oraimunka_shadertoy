...
void writeimage(const int width, const int height, const vec3* data) {
	...
	for (int y = 0; y < height; y++)
	{
		for (int x = 0; x < width; x++) {
			...
			fin[offset + 0] = (unsigned char)clamp(static_cast<int>(rgb[0] * 255.f), 0, 255);
			fin[offset + 1] = (unsigned char)clamp(static_cast<int>(rgb[1] * 255.f), 0, 255);
			fin[offset + 2] = (unsigned char)clamp(static_cast<int>(rgb[2] * 255.f), 0, 255);
		}
	}
